    ...
    public static class FightRiskAlertUtility
	{
		public static List<Pawn> PawnsAtRiskExtreme
		{
			get
			{
				...
				{
					if (!pawn.Downed && pawn.needs.TryGetNeed(OrkNeedDefOf.OG_Ork_Fightyness) is Need_Orkoid_Fightyness need_Violence && need_Violence.CurCategory <= FightynessCategory.Requires)
					{
		...
		public static List<Pawn> PawnsAtRiskMajor
		{
			get
			{
				...
				foreach (Pawn pawn in PawnsFinder.AllMapsCaravansAndTravelingTransportPods_Alive_FreeColonists_NoCryptosleep)
				{
					if (!pawn.Downed && pawn.needs.TryGetNeed(OrkNeedDefOf.OG_Ork_Fightyness) is Need_Orkoid_Fightyness need_Violence && need_Violence.CurCategory <= FightynessCategory.Desires)
					{
		...
		public static List<Pawn> PawnsAtRiskMinor
		{
			get
			{
				...
				foreach (Pawn pawn in PawnsFinder.AllMapsCaravansAndTravelingTransportPods_Alive_FreeColonists_NoCryptosleep)
				{
					if (!pawn.Downed && pawn.needs.TryGetNeed(OrkNeedDefOf.OG_Ork_Fightyness) is Need_Orkoid_Fightyness need_Violence && need_Violence.CurCategory < FightynessCategory.Free)
					{
